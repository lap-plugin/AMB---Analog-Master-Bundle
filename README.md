...
| **AMB-GE15** | 15-Band Graphic EQ (⅔-oct raster)               | ±12 dB / 1 dB steps |
| **AMB-GE31** | 31-Band Mastering Graphic EQ (⅓-oct ISO)        | ±6 dB / 0.5 dB steps |
...
- **Drive constant** (`0.4` for PE3/PE5/PE8, `0.25` for GE15/GE31/BC/MBC) —
...
- **State Variable Filter** (Cytomic / Simper-style; one canonical
  `SvfFilter` type since v2.2). Shelf · Bell · Lowpass · Highpass · Butterworth.
  NaN protection built in.
- **`EqSection<const N: usize>`** — generic EQ scaffold in `lap-dsp` since
  v3.1. Holds the filter array (`[[SvfFilter; N]; 2]`) and the 2× IIR
  oversampler. PE3/PE5/PE8/GE15/GE31 all use this — one source of truth for
  EQ structure across the bundle. Each plugin only writes its own
  `update_coeffs` (the per-band shape logic).
...
- **Proportional-Q** *(PE3/PE5/PE8)* — Q widens with low gain (musical)
...
- **Per-band shape variety** — every EQ band has 2 or 3 shape options.
  Mid bands choose between `Bell` / `Low Shelf` / `High Shelf`. Edge bands
  add `Highpass` (lowest band) or `Lowpass` (highest band) at fixed
  12 dB/oct Butterworth slope, with the freq selector as cutoff. To disable
  a band, set its gain to 0 dB — the natural workflow.

### DC + Limiting

- **Sample-rate-correct DC blocker** — corner pinned to ~5 Hz at any
  sample rate (44.1k–192k). Previously the corner drifted to ~15 Hz @ 192k.
- **Soft limiter on MBC output** — transparent below ~-0.9 dBFS, smooth
...
|-------|----------------------------|----------------------------------------------|
| LOW   | 30 / 50 / 80 / 120 Hz      | Low Shelf · Bell · Highpass *(12 dB/oct)*    |
| MID   | 1 / 2 / 3 / 5 kHz          | Bell · Low Shelf · High Shelf                |
| HIGH  | 10 / 12 / 15 / 20 kHz      | High Shelf · Bell · Lowpass *(12 dB/oct)*    |

### PE5 (5 bands, overlapping ranges)

| Band     | Frequencies                              | Shape options                                |
|----------|------------------------------------------|----------------------------------------------|
| SUB      | 25 / 30 / 40 / 50 / 60 / 80 Hz           | Low Shelf · Bell · Highpass *(12 dB/oct)*    |
| LOW      | 100 / 120 / 180 / 250 / 350 / 500 Hz     | Bell · Low Shelf · High Shelf                |
| MID      | 1 / 1.5 / 2 / 3 / 5 / 7 kHz              | Bell · Low Shelf · High Shelf                |
| MID HIGH | 3 / 5 / 7 / 10 / 12 / 15 kHz             | Bell · Low Shelf · High Shelf                |
| HIGH     | 10 / 12 / 15 / 18 / 20 / 22 kHz          | High Shelf · Bell · Lowpass *(12 dB/oct)*    |

### PE8 (8 bands, mastering)

| Band       | Frequencies                                            | Shape options                                |
|------------|--------------------------------------------------------|----------------------------------------------|
| SUB        | 20 / 25 / 30 / 40 / 50 / 60 / 80 / 100 Hz              | Low Shelf · Bell · Highpass *(12 dB/oct)*    |
| LOW        | 50 / 60 / 80 / 100 / 120 / 150 / 180 / 250 Hz          | Bell · Low Shelf · High Shelf                |
| MID LOW    | 250 / 350 / 500 / 700 / 1k / 1.2k / 1.5k / 2k Hz       | Bell · Low Shelf · High Shelf                |
| MID        | 1.5 / 2 / 2.5 / 3 / 4 / 5 / 7 / 8 kHz                  | Bell · Low Shelf · High Shelf                |
| MID HIGH   | 2.5 / 3 / 5 / 7 / 8 / 10 / 12 / 15 kHz                 | Bell · Low Shelf · High Shelf                |
| HIGH       | 10 / 12 / 15 / 18 / 20 / 22 / 25 / 28 kHz              | High Shelf · Bell                            |
| AIR        | 12 / 15 / 18 / 20 / 22 / 25 / 28 / 32 kHz              | High Shelf · Bell                            |
| AIR ULTRA  | 15 / 18 / 20 / 22 / 25 / 28 / 32 / 40 kHz              | High Shelf · Bell · Lowpass *(12 dB/oct)*    |
...
└── crates/
    ...
    │       ├── eq_section.rs        # generic EqSection<N> shared by all EQ plugins
    ...
    ├── ambge15/              # 15-band graphic EQ (⅔-octave)
    ├── ambge31/              # 31-band mastering graphic EQ (⅓-octave ISO)
...
cargo xtask bundle ambge15 --release
cargo xtask bundle ambge31 --release
...
cp target/release/libambge15.so ~/.clap/AmbGE15.clap
cp target/release/libambge31.so ~/.clap/AmbGE31.clap
...
# (analogically for ambpe5, ambpe8, ambge15, ambge31, ambbc, ambmbc)
```
...
## v2.7 changes (historical detail)
...
- **Per-band shape options** — every EQ band has 2–3 shape choices instead
  of an `Off / shape` toggle. Mid bands offer `Bell / Low Shelf / High Shelf`;
  edge bands also include `Highpass` (lowest band) or `Lowpass` (highest band)
  at fixed 12 dB/oct Butterworth slope. The natural way to "disable" a band
  is to set its gain to 0 dB — no separate Off state needed.

### Analog character — "always on" model

The `if analog_enabled` guard around the background noise generator was
**removed in all five plugins** (PE3, PE5, GE15, BC, MBC; PE8 ships with
...
- **v3.1 (current)** — DSP consolidation. Extracted `EqSection<const N: usize>`
  into `lap-dsp` — previously 5 plugins (PE3/PE5/PE8/GE15/GE31) had their own
  near-identical copies of the same EQ scaffolding (filter array + 2× IIR
  oversampler + reset + process_one). Now a single generic source of truth.
  Each plugin keeps its own `update_coeffs` (the part that genuinely differs
  per-plugin: per-band shape match arms in PE, bell cycle in GE) and snapshot
  detection. PE3/PE5 additionally hold bandpass HP+LP filters as separate
  fields (the only EQ-extra feature in the bundle). Also removed dead
  `FilterShape::Off` variant from `common.rs` (no plugin has used it since v2.8).
- **v3.0** — Added AMB-GE31, full ⅓-octave ISO mastering graphic
  EQ (31 bands, 20 Hz – 20 kHz, ±6 dB / 0.5 dB stepped, fixed Q=4.32).
  Designed for surgical mastering work where ±12 dB on GE15 would be too
  much. Bundle is now: PE3, PE5, PE8, GE15, GE31, BC, MBC.
- **v2.9** — Replaced AMB-GE10 (10-band, octave raster) with
  AMB-GE15 (15-band, ⅔-octave raster), stepped gain ±12 dB / 1 dB, fixed
  Q=2.14 (constant-Q). GE10 fully removed from the bundle.
- **v2.8** — Removed `Off` shape state from PE3/PE5/PE8. Each band now has
  2–3 shape choices (Bell / Low Shelf / High Shelf, plus Highpass on the
  lowest band and Lowpass on the highest band, both at fixed 12 dB/oct).
  Disable a band via gain = 0 dB.
- **v2.7** — PE8 added, stepped freqs/gains, proportional-Q, "always on"
